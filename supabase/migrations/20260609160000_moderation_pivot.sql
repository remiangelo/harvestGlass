...
--
-- Reflects production (project jutzlxdboayvmcuqwodn): the view is dropped and
-- recreated (column set changed vs the prior admin/schema.sql view, so
-- create-or-replace cannot be used) and created WITH (security_invoker = true).
...
create trigger community_messages_contact_guard before insert on public.community_messages
...
select r.id, r.reporter_id, r.reported_id, r.reason, r.description, r.status, r.action_taken, r.created_at,
  r.target_type, r.target_id,
  reported.nickname as reported_nickname, reported.bio as reported_bio, reported.photos as reported_photos,
  reported.is_banned as reported_is_banned, reporter.nickname as reporter_nickname,
  cm.content as target_message_content, cm.community_id as target_community_id, comm.name as target_community_name
...
left join public.community_messages cm on r.target_type = 'community_message' and cm.id = r.target_id
